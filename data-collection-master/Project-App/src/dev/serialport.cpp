                                ...
                                (sizeof(s->uart_receive.data_buffer) - s->uart_receive.temp_ptr))) > 0)
    {
      ...
      if (s->uart_receive.temp_ptr)
      {
        ...
        if ( strstr((char*)s->uart_receive.data_buffer, "AT+") != NULL || f->ATflag == 1 )    
        {
          ...
          f->ATflag = 1;
          memcpy(f->ATbuf + f->ATlen, s->uart_receive.data_buffer, s->uart_receive.temp_ptr);
          f->ATlen += s->uart_receive.temp_ptr;
          if ( strstr((char*)f->ATbuf, "\r\n") != NULL)    //接收完毕
          {
            f->ATflag = 2;
...
int SerialPort::ctrl(int value)
{
  ...
  else if (strcmp(this->COM, "/dev/ttyRS1") == 0)
  {
    io = "/dev/rs485Ctrl2";
    ;
  }
